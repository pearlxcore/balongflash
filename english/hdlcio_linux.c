...
if (masslen != 0) {
 ...
 if (res != (masslen-1)) {
   printf("\nModem response too short: %i byte, was expected %i byte\n",res+1,masslen);
...
if (strspn(dentry->d_name+3,"0123456789AaBbCcDdEeFf") != 8) {
  printf("\nSection ID Error - Non-numeric Character - %s\n",filename);
  exit(1);
}  
...
if (pt == 0xa55aaa55) {
  printf("\nThe file %s has a header - is not suitable for firmware\n",filename);
  exit(1);
}
