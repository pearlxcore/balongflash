...
if (feof(in)) {
  printf("\nNo sections found in the file - the file does not contain a firmware image\n");
  exit(0);
}  
...
do {
  printf("\rSearch section # %i",npart); fflush(stdout);	
...
if (signsize == -1) printf("\nDigital signature: not found");
  ...
  printf("\nDigital signature       : %i byte",signsize);
...
for (npart=0;npart<30;npart++) {
    ...
    if (ptable[npart].pimage == 0) {
      printf("\nMemory allocation error, section #%i, The size = %i byte\n",npart,ptable[npart].hd.psize);
      exit(0);
    }
