...
printf("\nIncorrect firmware monitor version - [%i]%s\n",res,replybuf+2);
// dump(replybuf,res,0);
return -1;
}
...
printf("\nPartitioning from the firmware file:\n\n## offset  Size   Name\n-------------------------------------");
for (i=0;i<npart;i++) {  
...
if (res != 6) {
  printf("\nIncorrect response length for ^DATAMODE");
  exit(-2);
}  
if (memcmp(replybuf,OKrsp,6) != 0) {
  printf("\nThe ^DATAMODE command is rejected, digital signature may be required\n");
  exit(-2);
}  
}
...
printf("\nTitle                   : Version %i, compliance code: %8.8s",ptable[0].hd.hdversion,ptable[0].hd.unlock);
}
